...
private:
	...
	bool TryBlockOnce(RowGroupScanSourceInput &input) {
		const annotated_lock_guard<annotated_mutex> guard {block_state.lock};
		if (has_blocked || !input.interrupt_state) {
			return false;
		}
		has_blocked = true;
		stats.blocks++;
		if (!block_state.BlockTask(*input.interrupt_state)) {
			// the scan can no longer block - proceed instead
			return false;
		}
		unblock_thread = std::thread([this]() {
			std::this_thread::sleep_for(std::chrono::milliseconds(20));
			const annotated_lock_guard<annotated_mutex> g {block_state.lock};
			block_state.UnblockTasks();
		});
